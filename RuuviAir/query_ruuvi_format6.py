#!/usr/bin/env python3
"""
Ruuvi Database Query Tool (Format 6)
Query and analyze stored Ruuvi Format 6 measurements
"""
...
def print_measurements(measurements: List[Dict[str, Any]]):
    ...
    for m in measurements:
        ...
        if m.get('measurement_sequence') is not None:
            print(f"Sequence: {m['measurement_sequence']}")
        
        if m.get('calibration_in_progress'):
...
def print_statistics(stats: Dict[str, Any], hours: int):
    ...
    if stats['avg_pm25'] is not None:
        ...
        print(f"  Average: {stats['avg_pm25']:.1f} µg/m³")
        print(f"  Min: {stats['min_pm25']:.1f} µg/m³")
        print(f"  Max: {stats['max_pm25']:.1f} µg/m³")
...
def main():
    """Main function"""
    parser = argparse.ArgumentParser(description='Query Ruuvi Format 6 database')
